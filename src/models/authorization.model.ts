...
type AuthorizationStatic = typeof Model & {
  new ( values?: Record<string, unknown>, options?: BuildOptions ): Authorization;
};

const AuthorizationFactory = ( sequelize : Sequelize ): AuthorizationStatic => {
  const attributes = {
    id: {
  ...
  const Authorization = <AuthorizationStatic>sequelize.define( 'authorizations', attributes, { 
    paranoid: true, 
    underscored: true, 
    comment: '토큰 정보',
  });
